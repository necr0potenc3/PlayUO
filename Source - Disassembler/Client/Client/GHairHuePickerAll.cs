namespace Client
{
    ...
    public class GHairHuePickerAll : Gump
    {
        ...
        public GHairHuePickerAll(int X, int Y, int Width, int Height) : base(X, Y)
        {
            do
            {
                this.xSize++;
            ...
            for (int i = 0; i < 6; i++)
            {
                for (int j = 0; j < 1; j++)
                {
                    for (int k = 0; k < 8; k++)
                    {
                        ushort num4 = Hues.GetData((0x44d + (((j * 6) + i) * 8)) + k).colors[0x30];
        ...
        public int Hue
        {
            get
            {
                return ((0x44e + (((this.m_yShade * 6) + this.m_xShade) * 8)) + this.m_zShade);
            }
        }
