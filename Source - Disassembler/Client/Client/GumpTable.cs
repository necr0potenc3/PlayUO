namespace Client
{
    ...
    {
        public static GumpTableEntry[] m_Entries;

        static GumpTable()
        {
            string path = Engine.FileManager.ResolveMUL("Gump.def");
            if (!File.Exists(path))
            {
                m_Entries = new GumpTableEntry[0];
            }
            else
            {
                string str2;
                ...
                while ((str2 = reader.ReadLine()) != null)
                {
                    if (((str2 = str2.Trim()).Length != 0) && !str2.StartsWith("#"))
                    {
                        try
                        {
                            ...
                            list.Add(new GumpTableEntry(oldID, newID, newHue));
                            continue;
                        }
                        catch
                        {
